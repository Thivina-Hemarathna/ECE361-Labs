...
#define INITAL_TIMEOUT_SECONDS 1
...
int main(int argc, char *argv[]) {
    ...
    start = clock();
    sendto(sockfd, "ftp", sizeof("ftp"), 0, (struct sockaddr *) &serverAddr, sizeof(serverAddr));
    ssize_t ack_size = recvfrom(sockfd, buffer, BUFFER_SIZE, 0, NULL, NULL);
    end = clock();

    round_trip_time = ((double)(end - start))/ CLOCKS_PER_SEC;
    printf("%lf\n",round_trip_time);

    struct timeval tv;
    //tv.tv_sec = 10*round_trip_time;
    tv.tv_sec = 0;
    tv.tv_usec = round_trip_time * 2 * 1000000;
    ...
    while((bytes_read = fread(pkt.filedata, 1, sizeof(pkt.filedata), file)) > 0){
        ...
        while(attempts < MAX_ATTEMPTS && !ack_received){
            ...
            ack_size = recvfrom(sockfd, buffer, BUFFER_SIZE, 0, NULL, NULL);
            if(ack_size > 0 && strncmp(buffer, "ACK", 3) == 0){
                printf("ACK received for fragment %u\n", pkt.frag_no);
                ack_received = 1;
            }else if(ack_size < 0 && errno == EWOULDBLOCK){
                fprintf(stderr, "Timeout waiting for ACK for %u, attempt %d\n", pkt.frag_no, attempts+1);
                attempts++;
            }else{
